(function($) {
	var
		...
		get_array_current_full = function () {
			return [get_current('y'),get_current('m'),get_current('d')];
        },
		...
		picker_render_calendar = function() {
			...
			var
				_C = new persianDate(get_array_current_full()),
				_S = new persianDate(get_array_current_full()),
				_L = new persianDate(get_array_current_full()),
				_NUM = function(d){
					var
						m = d.month() - 1,
						y = d.year()
                    	kabise = [0,4,8,12,16,20,24,29,33,37,41,45,49,53,57,62,66,70,74,78,82,86,90,95,99,103,107,111,115,119,124],
                        l = kabise.indexOf(y%128) >= 0;
					return [31, 31, 31, 31, 31, 31, 30, 30, 30, 30, 30, (l ? 30 : 29)][m];
				};
		...
		picker_fills = function() {
			...
			pickers[picker.id].key['d'].max =  [31, 31, 31, 31, 31, 31, 30, 30, 30, 30, 30, (l ? 30 : 29)][m-1];
			...
			.each(function() {
				var
					d = new persianDate([y,m,eval($(this).attr('value'))]).day() - 1;
			...
			if(picker.element.hasClass('picker-lg')) {
				...
				.each(function() {
					var
						d = new persianDate([y,m,eval($(this).attr('data-value'))]).day() - 1;
					if(d==6)
						$(this).addClass('pick-wke');
		...
		input_change_value = function() {
			...
			if(!is_locked()&&picker_ctrl) {
				...
				var
					...
					m = get_current('m'),
					y = get_current('y'),
					get_day = new persianDate([y,m,d]).day() - 1,
		...
		return $(this).each(function(){
			if($(this).is('input')&&!$(this).hasClass('picker-input')) {
				...
				var
					...
					picker_max_year = (input.data('max-year')&&is_int(input.data('max-year'))) ? input.data('max-year') : new persianDate().year() + 10,
					picker_min_year = (input.data('min-year')&&is_int(input.data('min-year'))) ? input.data('min-year') : 1300,
				...
				pickers[id] = {
					...
					key : {
						m : {
							...
							max : 12,
							current : 1,
							today : (new persianDate().month())
						},
						d : {
							min : 1,
							max : 31,
							current : 1,
							today : new persianDate().date()
						},
						y : {
							min : picker_min_year,
							max : picker_max_year,
							current : picker_min_year,
							today : new persianDate().year()
						},
						l : {
							min : 0,
